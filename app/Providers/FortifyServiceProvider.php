    ...
    public function register(): void
    {
        ...
        $this->app->singleton(LoginResponse::class, function () {
            return new class implements LoginResponse {
                ...
                {
                    // 1. Tambahkan Type-Hinting untuk Guard (objek auth())
                    /** @var \Illuminate\Contracts\Auth\StatefulGuard $guard */
                    $guard = auth();

                    // 2. Sekarang panggil user() pada objek yang sudah di-type-hint
                    $user = $guard->user(); // Ganti auth()->user() dengan $guard->user()
                    ...
                    return redirect()->intended(route('staff.absen'));
                }
            };
        });
        ...
        $this->app->singleton(RegisterResponse::class, function () {
            return new class implements RegisterResponse {
                public function toResponse($request)
                {
                    ...
                    // Default to staff dashboard
                    return redirect()->route('staff.absen');
                }
            };
        });
    }
